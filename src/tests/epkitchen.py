...
def evaluate_human(tool_place, init, policy, exp_type):
    prev_loc = init
    rewards = 0
    counter = policy["counter"]
    cupboards = policy["cupboards"]
    oven = policy["oven"]
    door = policy["door"]
    fridge = policy["fridge"]
    tool_x = tool_place[0]
    tool_y = tool_place[1]
    if counter[0] == tool_x and counter[1] == tool_y:
        rewards += print_moves(prev_loc, counter)
        prev_loc = counter
    else:
        if exp_type == "B":
            rewards += print_moves(prev_loc, cupboards[0])
            prev_loc = cupboards[0]
            if cupboards[0][0] == tool_x and cupboards[0][1] == tool_y:
                print("Agent Location:", prev_loc[0], prev_loc[1], "Action:", "Do")
                rewards += -1
            else:
                print("Agent couldn't find the utensils")
                rewards = -50
                print("Total Reward:", rewards)
                return
        else:
            for cup in cupboards:
                rewards += print_moves(prev_loc, cup)
                prev_loc = cup
                print("Agent Location:", prev_loc[0], prev_loc[1], "Action:", "Do")
                rewards += -1
                if cup[0] == tool_x and cup[1] == tool_y:
                    print("Agent Location:", prev_loc[0], prev_loc[1], "Action:", "Do")
                    rewards += -1
                    break
...
def generate_policies(map_fn, exp_type):
    map = load_map(map_fn)
    oven = None
    counter = None
    cupboard = None
    ...
    do_locs = [tool, oven, cupboard]
    if exp_type == "C":
        return [policy2, policy1], do_locs

    return [policy1, policy2], do_locs

def train(filename, seed, exp_type):
    ...
    tasks = [[OBJECTS1["food"], OBJECTS1["tidy"]]]
    not_task = [OBJECTS1["tool"]]
    ...
    with open(filename, "w") as csvfile:
        ...
        for j, goal in enumerate(tasks):
            ...
            humans_policy1, do_locs1 = generate_policies(map_fn, exp_type)

            humans_policy2, do_locs2 = generate_policies(map_fn, exp_type)

            reward2 = ReachFacts(env2, goal, not_task)
            policy2 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.4,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng1, others_policy=humans_policy1, others_init=[[1,3]], others_dist=[0.9, 0.1], penalty=-2*EPISODE_LENGTH, others_alpha=[1.01, 1.01], objects=OBJECTS1, problem_mode=problem_mode)
            ...
            policy3 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.4,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng3, others_policy=humans_policy2, others_init=[[1,3]], others_dist=[0.9, 0.1], penalty=-2*EPISODE_LENGTH, others_alpha=[1.01, 1.01], objects=OBJECTS1, problem_mode=problem_mode, full_observability = True)
            ...
            try:
                ...
                agent3.train(steps=TOTAL_STEPS3,
                             steps_per_episode=EPISODE_LENGTH, report=report3)

                print("Default Human:")
                humans,_ = generate_policies(map_fn, exp_type)
                evaluate_human(humans[0]["cupboards"][0], [1,3], humans[0], exp_type)
                print("-----------------")
                print("Non-Augmented Robot:")
                base_step, last_state = evaluate_agent(env1, policy1, reward1, init1)
                print("Total Reward = ", base_step)
                print("-----------------", last_state.tool_x, last_state.tool_y)
                print("Human:")
                humans,_ = generate_policies(map_fn, exp_type)
                evaluate_human([last_state.tool_x, last_state.tool_y], [1,3], humans[0], exp_type)
                print("-----------------")
                print("Our Robot:")
                ...
                print("Human:")
                humans,_ = generate_policies(map_fn, exp_type)
                evaluate_human([last_state2.tool_x, last_state2.tool_y], [1,3], humans[0], exp_type)
                print("-----------------")
                ...
                print("Human:")
                humans,_ = generate_policies(map_fn, exp_type)
                evaluate_human([last_state3.tool_x, last_state3.tool_y], [1,3], humans[0], exp_type)
                print("-----------------")
