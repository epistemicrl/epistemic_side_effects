...
from environment.kitchen import Kitchen, KitchenState, OBJECTS2, update_facts, MAPPING
...
def evaluate_human(wet, init, policy):
    prev_loc = init
    rewards = 0
    door = policy["door"]
...
def train(filename, seed):
    ...
    map_fn = path.join(here, "maps/wetkitchen.map")
    rng1 = Random(seed + 1)
    ...
    with open(filename, "w") as csvfile:
        ...
        for j, goal in enumerate(tasks):
            ...
            policy2 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.4,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng1, others_policy=humans_policy, others_init=[[1,3]], others_dist=[0.25, 0.75], penalty=-2*EPISODE_LENGTH, others_alpha=[1.01, 1.01], objects=OBJECTS2, problem_mode=problem_mode)
            ...
            policy3 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.4,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng3, others_policy=humans_policy, others_init=[[1,3]], others_dist=[0.25, 0.75], penalty=-2*EPISODE_LENGTH, others_alpha=[1.01, 1.01], objects=OBJECTS2, problem_mode=problem_mode, full_observability=True)
            ...
            try:
                ...
                print("Human:")
                humans= generate_policies(map_fn)
                evaluate_human(last_state1.facts[1], [1, 3], humans[0])
                print("-----------------")
                print("Our Robot:")
                ...
                print("Human:")
                humans = generate_policies(map_fn)
                evaluate_human(last_state2.facts[1], [1, 3], humans[0])
                print("-----------------")
                ...
                print("Human:")
                humans = generate_policies(map_fn)
                evaluate_human(last_state3.facts[1], [1, 3], humans[0])
